<div align="center">

# 🀄️ 麻将棋牌大厅源码 | 麻將棋牌大廳源碼 | Mahjong Game Hall Source Code

**血流成河 · 血战到底 · 二人麻将 · 广东麻将**

[![Contact](https://img.shields.io/badge/联系-Telegram-blue.svg)](https://t.me/alibabama401)

**简体中文 · 繁體中文 · English**

</div>

---

## 📖 产品简介 | 產品簡介 | Overview

| 语言 | 说明 |
|:---|:---|
| **简体中文** | 一套**完整的麻将棋牌大厅源码**，包含**源代码**和**美术资源**（可分别出售）。支持多种麻将玩法：**血流成河、血战到底、二人麻将、广东麻将**等。 |
| **繁體中文** | 一套**完整的麻將棋牌大廳源碼**，包含**原始碼**和**美術資源**（可分別出售）。支援多種麻將玩法：**血流成河、血戰到底、二人麻將、廣東麻將**等。 |
| **English** | A **complete Mahjong game hall source code**, including **source code** and **art resources** (can be sold separately). Supports multiple Mahjong games: **Bloody River, Blood Battle, Two-Player Mahjong, Guangdong Mahjong**, and more. |

---

## ✨ 包含内容 | 包含內容 | What's Included

| 类型 | 说明 |
|:---|:---|
| 💻 **源代码** | 完整的麻将游戏大厅源代码 |
| 🎨 **美术资源** | UI、牌面、特效等全套美术资源 |
| 🀄️ **多种玩法** | 血流成河、血战到底、二人麻将、广东麻将 |
| 🏠 **大厅系统** | 完整的游戏大厅功能 |

---

## 🎮 麻将玩法 | 麻將玩法 | Game Modes

| 玩法 | 说明 |
|:---|:---|
| 🩸 **血流成河** | 经典血流成河麻将 |
| ⚔️ **血战到底** | 血战到底麻将规则 |
| 👥 **二人麻将** | 1v1 对战麻将 |
| 🀄️ **广东麻将** | 广东地区麻将规则 |

---

## 🏗️ 技术架构 | Tech Stack

| 模块 | 技术 |
|:---|:---|
| 客户端语言 | Lua |
| 游戏引擎 | 主流游戏引擎 |
| 服务端 | 配套服务端（可咨询） |

---

## 📸 界面截图 | Screenshots

> **请将你的游戏截图拖拽到下方**（建议：游戏大厅、血流成河、广东麻将各一张）

| 游戏大厅 | 血流成河 | 广东麻将 |
|:---:|:---:|:---:|
![廖海良_2018-3-15_麻将商城界面](https://github.com/user-attachments/assets/f939fb3c-4c3c-4819-a16d-1e63422eba5d)
![s效果图](https://github.com/user-attachments/assets/0322e47b-b7e1-4168-b08b-ad593a9bfc85)
![_2018-2-7_麻将主大厅](https://github.com/user-attachments/assets/6fcb3730-dff8-4eda-9a40-b4e9fc9e9bf2)
![效果图2-1](https://github.com/user-attachments/assets/e2ecb50b-4a03-479d-b43f-1b54d6ac9667)
![效果图-1](https://github.com/user-attachments/assets/f3a728b9-572d-431f-8e58-efbb8e8ed22c)
![效果图](https://github.com/user-attachments/assets/fd76e4e3-b1b3-4262-8f3f-1c6867b9ec7d)
![牌桌换桌](https://github.com/user-attachments/assets/4fd7c070-1da4-469e-abf5-af3a407750d2)
![念凯文_2018-4-10_麻将牌局界面_花牌数量显示](https://github.com/user-attachments/assets/653239d1-a2bf-4410-821d-54ab0e104cb6)
![念凯文_2018-2-7_结算界面-二人-胜利02](https://github.com/user-attachments/assets/86ff50f2-37ac-4ed3-9b7e-419c245521de)
---

## 📦 出售方式 | 出售方式 | Purchase Options

| 选项 | 说明 |
|:---|:---|

| 📦 **全套购买** | 源代码 |

---

## 🚀 快速体验 | Quick Start

### 获取完整源码/资源

如需获取**麻将棋牌大厅源码或美术资源**，请联系：

| 渠道 | 账号 |
|:---|:---|
| **Telegram** | @alibabama401 |
| **Email** | ttpoker733@gmail.com |

---

## ❓ 常见问题 | FAQ

**Q1：这套源码包含哪些麻将玩法？**  
A：包含血流成河、血战到底、二人麻将、广东麻将等。

**Q2：源代码和美术资源可以分开买吗？**  
A：可以。源代码和美术资源分别出售，按需购买。

**Q3：美术资源包含什么？**  
A：包含UI界面、牌面、特效、场景等全套美术资源。

**Q4：可以二次开发吗？**  
A：可以。源码支持功能扩展。

**Q5：提供部署服务吗？**  
A：提供。购买后可远程协助部署。

---

<div align="center">

**如果这个项目对你有帮助，欢迎点个 Star 支持一下！**  
⭐️ **Star** ⭐️

</div>

